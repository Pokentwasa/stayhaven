...
/**
 * 02 — Brand Manifesto. A quiet, pinned breathing moment between the House
 * and the Experience: one line of the manifesto fills the viewport at a
 * time, dissolving into the next as the user scrolls, over a faint
 ...
 * imperatively at runtime only when motion is allowed.
 */
export function BrandManifesto({ property }: { property: Property }) {
  ...
  return (
    ...
    >
      <div ref={bgRef} className="absolute inset-0 opacity-[0.06]">
        <PlaceholderMedia media={property.heroMedia} className="absolute inset-0 grayscale" />
      </div>
