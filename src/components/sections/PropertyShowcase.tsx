...
/**
 * 01 — The Collection. The signature moment of the homepage: a pinned,
 * layered editorial sequence. Each property image wipes over the last
 * (clip-path reveal, not a hard cut) inside a large inset frame, with an
 * oversized page-slug numeral and the title block crossfading on their own,
 * offset rhythm. Mobile drops the pin for a vertical cinematic stack.
 */
export function PropertyShowcase() {
  ...
  useEffect(() => {
    ...
    if (prefersReducedMotion()) {
      // Static, accessible fallback: show the first property, all reachable via normal scroll.
      setInteractive(0);
      return;
    }
  ...
  return (
    ...
    >
          ...
          {PROPERTIES.map((property, i) => (
            ...
            >
              ...
              <div className="mt-2 flex items-center gap-10">
                <LuxuryButton href={`/stays/${property.slug}`} variant="primary" data-cursor="VIEW">
                  Explore Property
                </LuxuryButton>
      ...
      <div className="flex flex-col md:hidden">
        {PROPERTIES.map((property, i) => (
          <article key={property.id} data-mobile-panel className="relative flex h-[92svh] w-full flex-col justify-end">
            <div data-frame-media className="absolute inset-0 h-full w-full">
              <PlaceholderMedia media={property.heroMedia} className="absolute inset-0" />
            </div>
